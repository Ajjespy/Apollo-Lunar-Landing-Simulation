...
public:
    Demo(const Point& ptUpperRight) :
            ...
            ptLM(ptUpperRight.getX() / 2.0, ptUpperRight.getY() - 20),
            ground(ptUpperRight)
...
void callBack(const Interface *pUI, void * p)
{
   ...
   double fuel = 4.0;
   double altitude = pDemo->ground.getElevation(pDemo->ptLM);
   double speed = 30.0;
   gout.setf(ios::fixed | ios::showpoint);
   gout.precision(2);
   gout.setPosition(Point(5.0, 385.0));
   ...
   for (int i = 0; i < 10; i++)
   {
       for (int j = 0; j < 5; j++)
       {
           pDemo->ptStar.addY(-10.0);
           gout.drawStar(pDemo->ptStar, 255);
       };
       pDemo->ptStar.addY(+50.0);
       pDemo->ptStar.addX(-30.0);
   };
